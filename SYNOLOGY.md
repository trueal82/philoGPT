...
- Your domain or DDNS hostname pointed at the NAS, with the following subdomains:
  ...
  - `philogpt-api.example.com` — API service
...
| `API_URL` | Public URL of the API service — `https://philogpt-api.example.com` |
...
> **Why `FRONTEND_URL` / `ADMIN_URL`?** Compose injects them into `ALLOWED_ORIGINS` on the API, so CORS is configured automatically from the same values. `API_URL` flows into both frontend services as `API_URL`/`SOCKET_URL` — the browser uses it to reach the API, so it must be a public domain, not a Docker-internal hostname.

**Optional overrides (defaults shown):**

| Variable | Default | Notes |
|---|---|---|
| `API_PORT` | `5001` | Host port for the API — change if it conflicts with DSM or other services |
...
```
NAME                          STATUS          PORTS
philogpt-API-1            running         0.0.0.0:5001->5001/tcp
...
Check API logs to confirm the database seeded successfully:
...
| Target server | `http://localhost:3002` |
| WebSocket support | ✓ (required for real-time chat) |

---

### 4.3 API Service — `philogpt-api.example.com` (port 5001)

The API is called by the browsers directly, so it needs its own public hostname:
